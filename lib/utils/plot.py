...
def show_seg_result(img, result, index, epoch, save_dir=None, is_ll=False,palette=None,is_demo=False,is_gt=False):
    ...
    print('\nColor mask:',color_mask.shape)
    print('\nimg:',img.shape)
    #img= img.resize

    print('\nColor seg:',color_seg.shape)
    #pdb.set_trace()
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None, depth = None):
    ...
    # Extract depth from depth matrix
    depth_value = depth[center_y, center_x]
    # cv2.rectangle(img, c1, c2, color, thickness=tl)
    depth_text = "Depth:{:.2f}".format(depth_value)
    ...
    if label:
        ...
        cv2.putText(img, "Class: "+str(label.split()[0])+'|Score'+ str(label.split()[1]), (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf,lineType= cv2.LINE_AA)
        
        cv2.putText(img, depth_text, (c1[0], c1[1] - t_size[1] - 9), 0, float(tl) / 3, [0, 0, 255],
                thickness=1, lineType=cv2.FONT_HERSHEY_SIMPLEX)
        if float(label.split()[1])<=1.0:
            text = '---Safe---'
            cv2.putText(img, text, (c1[0], c1[1] - t_size[1] - 20), 0, float(tl) / 3, [0, 255, 0],
                thickness=1, lineType=cv2.FONT_HERSHEY_SIMPLEX)
        else:
            text= '-Not Safe-'
            cv2.putText(img, text, (c1[0], c1[1] - t_size[1] - 20), 0, float(tl) / 3, [255, 0, 0],
                thickness=1, lineType=cv2.FONT_HERSHEY_SIMPLEX)
